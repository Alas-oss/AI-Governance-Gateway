...
def test_hr_senior_sees_unmasked_card_number_live(client, upstream, mint_token):
    """Regression test for KNOWN_BUG.md: an HR-senior user is exempt from
    CREDIT_CARD/BANK_CARD_NUMBER masking. The live response they receive
    must contain the real card number, not a masked placeholder."""
...
def test_hr_junior_gets_card_number_masked_live(client, upstream, mint_token):
    """Control case: an HR-junior user has no masking exemption, so the same
    card number must come back masked."""
...
def test_non_hr_senior_gets_card_number_masked_live(client, upstream, mint_token):
    """The masking exemption is HR+senior specific, not just senior."""
    upstream.set_response_content(f"On file: card {TEST_CARD_NUMBER}.")
...
def test_persisted_audit_log_masks_even_the_exempt_card_number(client, upstream, mint_token, monkeypatch):
    """The persisted-view guarantee: even for an HR-senior's own exempt
    request, whatever gets handed to the audit logger / cache must NOT
    contain the real card number -- only the live HTTP response is allowed
    to show it. This is the guarantee KNOWN_BUG.md said was independently
    verified and unaffected; confirming it still holds after the fix."""
    ...
    # Isolate this test from the semantic cache entirely -- it's about the
    # masking guarantee, not caching, and a near-duplicate question from an
    # earlier test could otherwise register as a cache hit.
    ...
    assert TEST_CARD_NUMBER in live_content  # sanity: live view still correct after the fix
