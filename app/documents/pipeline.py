...
def payload_reference_documents(payload: Dict[str, Any]) -> bool:
    messages = payload.get("messages")
    if not isinstance(messages, list):
        return False
    for message in messages:
        content = message.get("content") if isinstance(message, dict) else None
        if isinstance(content, str) and _DOCUMENT_REFERENCE_PATTERN.search(context):
            return True
    return False
