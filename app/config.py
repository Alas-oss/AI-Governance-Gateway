...
class Settings(BaseSettings):
    ...
    upstream_base_url: str = Field(
        ...
        descriptiong="Base URL of the internal AI Agent / LLM infrastructure the gateway proxies to.",
    )
    ...
    redis_url: str = Field(
        ...
        description="Redis connection URL backing the rate limiter and accounting.",
    )
    redis_max_connections: int = Field(default=50, ge=1)
    rate_limit_window_seconds: int = Field(
        default=60,
        ge=1, description="Sliding window size, in seconds, for the loop detector / rate limiter.",
    ...
    embedding_model_name: str = Field(default="all_miniLM-L6-v2")
    ...
    service_name: str = Field(Default="AI-Governance-Gate")
